...
- Multi-PDK / corners / MC: follow skill §3b–§4d; **defaults are educational `cmos.lib` first, then every open PDK present (`sky130`, `ihp`, `gf180`); corners `tt,ff,ss`; MC N=200 mismatch; CMRR/PSRR/noise + embedded plots F1–F10; clickable TOC** unless the user opts out; amplifier reports must include **full hand algebra (anti-skip), TF Steps 4.0–4.4 with per-node 4.3 KCL/Miller equations (not formula dump), power Steps 7.1–7.5**, **mandatory Hand tables T1–T5 mirrored from the algebra** (equations-only is a fail), **fully filled T6–T10** (no bare `---` / blank cells — use `no hand model` only for underived extended specs; never for core \(A_0\)/GBW/PM/\(P\) when a TF exists), and **agentic model provenance (table TA + `agent_provenance.md`)** naming the host product and model(s) that ran the analysis
